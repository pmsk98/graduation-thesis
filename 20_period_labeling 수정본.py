# -*- coding: utf-8 -*-
"""
Created on Tue Apr  6 15:20:42 2021
...
for i in range(len(df)):
    ...
    for year in range(2008, 2021):
        deleted.append(temp[('%d-01-01' %year <= temp['Date']) & (temp['Date'] < '%d-01-01' %(year+1))].iloc[:-19])
...
for i in range(0,73):
    for e in df[i].index:
        try:
            df[i]['new_return'][e] = df[i]['return'][e+20] - df[i]['return'][e+1]
        except:
            pass
